...
def ensemble():
    ...
    df_results = pd.DataFrame(columns=['KNN', 'DT', 'RF', 'XGB', 'MLP', 'LSTM'])
    ensemble_list = dict()
    for m in models:
...
def voter():
    ...
    ensemble_list = dict()
    for m in models:
        if tprs[m][results[m]['type']] > 0.45:
            ensemble_list[m] = results[m]['type']
        else:
            ensemble_list[m] = np.nan
    df_results = df_results.append(ensemble_list, ignore_index=True)
...
for msg in consumer:
    ...
    df_6 = df[['sbytes', 'dbytes', 'sload', 'smean', 'dmean', 'ct_srv_dst']]
    results = dict()
